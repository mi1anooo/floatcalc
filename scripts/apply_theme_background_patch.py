#!/usr/bin/env python3
"""Apply FloatCalc theme split + custom background image patch.

Run from the project root:
    python scripts/apply_theme_background_patch.py

This patch:
- Splits Glass and Frosted Glass into separate theme options.
- Adds an Image Background theme with user-uploaded images via FileReader.
- Makes native blur toggle only when the Frosted Glass theme is active.
- Keeps old Night/Dark/Glass themes intact.
"""
...
def read(path: str) -> str:
    p = ROOT / path
    if not p.exists():
        raise SystemExit(f"Missing file: {path}. Run this script from your project root.")
...
def write(path: str, text: str) -> None:
    ...
    backup = p.with_suffix(p.suffix + ".bak-theme-bg")
...
const THEME_OPTIONS: { value: AppTheme; label: string; desc: string; preview: string }[] = [
  ...
  { value: 'dark',    label: 'Dark',          desc: 'Neutral charcoal and blue',     preview: '#202020' },
  { value: 'glass',   label: 'Glass',         desc: 'Transparent glass, no blur',    preview: 'linear-gradient(135deg,rgba(80,60,140,0.58),rgba(30,28,60,0.45))' },
  { value: 'frosted', label: 'Frosted Glass', desc: 'Native blurred background',     preview: 'linear-gradient(135deg,rgba(196,181,253,0.38),rgba(26,26,30,0.52))' },
  { value: 'image',   label: 'Image',         desc: 'Use your own background image', preview: 'linear-gradient(135deg,#2b1b4a,#0f0f14)' },
];
...
export function SettingsDrawer({
  ...
  onChangeDefaultMode, onChangeCalcMode, onChangeTheme, onChangeBackgroundImage,
  onHideToTray,
}: SettingsDrawerProps) {
  ...
  const getThemePreview = (theme: AppTheme, fallback: string) => {
    if (theme === 'image' && settings.customBackgroundImage) {
      ...
      <div className="drawer" onClick={(e) => e.stopPropagation()}>
        ...
        <div className="drawer__body">
          ...
          <div className="drawer__theme-grid">
            {THEME_OPTIONS.map((t) => (
              <button
                ...
                onClick={() => {
                  if (t.value === 'image' && !settings.customBackgroundImage) {
                    handlePickImage();
                    return;
                  }
                  onChangeTheme(t.value);
                }}
              >
                <span
                  className="drawer__theme-swatch"
                  style={{ background: getThemePreview(t.value, t.preview) }}
                />
          ...
          <div className="drawer__image-tools">
            ...
            />
            <button className="drawer__image-btn" onClick={handlePickImage}>
              {settings.customBackgroundImage ? 'Change background image' : 'Upload background image'}
            </button>
            {settings.customBackgroundImage && (
              <button className="drawer__image-btn drawer__image-btn--danger" onClick={() => onChangeBackgroundImage(null)}>
                Remove image
              </button>
            )}
          </div>
...
def patch_app() -> None:
    ...
    # React import: add CSSProperties type if needed.
    text = re.sub(
        r"import \{ ([^}]+) \} from 'react';",
        lambda m: "import { type CSSProperties, " + m.group(1) + " } from 'react';" if "CSSProperties" not in m.group(1) else m.group(0),
        text,
        count=1,
    )

    if "from '@tauri-apps/api/tauri'" not in text:
        text = text.replace(
            "import { appWindow",
    ...
    if "customBackgroundImage" not in text.split("function uid", 1)[0]:
        text = text.replace(
            "  theme:        'night',\n};",
            "  theme:        'night',\n  customBackgroundImage: null,\n};",
        )

    # Replace theme application section.
    theme_section = r'''  // ── Theme application ───────────────────────────────────
  const applyTheme = useCallback((theme: AppTheme) => {
    document.documentElement.setAttribute('data-theme', theme);

    // Native desktop blur should only be active for Frosted Glass.
    // Regular Glass stays translucent but does not blur what is behind the window.
    void invoke('set_frosted_effect', { enabled: theme === 'frosted' }).catch(() => {
      // The UI should still work if native blur is unsupported on a platform.
    });
  }, []);
...
'''
    ...
    # If the above didn't match because applyTheme was already a useCallback, replace until Window sizing.
    text = re.sub(
        r"  // ── Theme application ─+\n.*?\n\s*// ── Window sizing",
        theme_section + "  // ── Window sizing",
        text,
        flags=re.S,
    )

    # Update handleChangeTheme and add image handler.
    replacement = r'''  const handleChangeTheme = useCallback((theme: AppTheme) => {
    applyTheme(theme);
  ...
  const handleChangeBackgroundImage = useCallback((imageDataUrl: string | null) => {
    ...
    # Add background style to app wrapper.
...
/* Glass is translucent only. It does not blur the desktop behind the app. */
[data-theme="glass"] {
...
/* Frosted Glass uses native window blur from Rust plus CSS polish inside the WebView. */
[data-theme="frosted"] {
...
/* User-uploaded background image theme. */
[data-theme="image"] {
...
def patch_css() -> None:
    ...
    # Clean old invalid duplicate transition variables if present.
...
def patch_rust() -> None:
    ...
    if config_path.exists():
        ...
        backup = config_path.with_suffix(".json.bak-theme-bg")
...
def main() -> None:
    ...
    print("FloatCalc theme/background patch applied.")
    print("Added separate Glass, Frosted Glass, and Image theme options.")
    print("Backups were created beside modified files with .bak-theme-bg suffix when possible.")
    print("Now run: npm run tauri dev")
